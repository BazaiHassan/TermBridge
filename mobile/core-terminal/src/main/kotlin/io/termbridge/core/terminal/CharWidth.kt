...
/**
 * Terminal cell width of a code point, like wcwidth(3): 2 for East Asian wide and fullwidth
 * characters and emoji, 0 for combining marks and other zero-width characters (they join the
 * previous cell), 1 for everything else. Wide ranges follow Unicode 15.1 EastAsianWidth.txt (W, F).
 */
object CharWidth {
    fun of(cp: Int): Int {
        if (cp < 0x300) return 1
        when (Character.getType(cp)) {
            Character.NON_SPACING_MARK.toInt(), Character.ENCLOSING_MARK.toInt() -> return 0
            Character.FORMAT.toInt() -> if (cp != 0xAD) return 0 // soft hyphen stays visible
        }
        if (cp in 0x1160..0x11FF || cp == 0x200B) return 0 // Hangul medial vowels, zero-width space
    ...
    private val WIDE = intArrayOf(
        ...
        0x2E80, 0x2E99, 0x2E9B, 0x2EF3, 0x2F00, 0x2FD5, 0x2FF0, 0x2FFF, 0x3000, 0x303E,
